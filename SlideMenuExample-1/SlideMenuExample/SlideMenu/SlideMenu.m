...
- (void)setLeftViewController:(UIViewController *)leftViewController {
    ...
    _leftViewController.view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [self menuWidth], self.view.frame.size.height)];
...
- (void)setRightViewController:(UIViewController *)rightViewController {
    ...
    //提前设置ViewController的viewframe，为了懒加载view造成的frame问题，所以通过setter设置了新的view
    _rightViewController.view = [[UIView alloc]initWithFrame:CGRectMake([self menuWidth], 0, [self menuWidth], self.view.frame.size.height)];
    //自定义View需要主动调用viewDidLoad
...
- (void)panChanged:(UIPanGestureRecognizer *)pan {
    ...
    if (CGRectGetMinX(_rootViewController.view.frame) > 0) {
        //显示左菜单 //http://blog.csdn.net/u012960049/article/details/56485125
        [self.view sendSubviewToBack:_rightViewController.view];//把——rightViewController放到父视图的最下面
        //更新左菜单位置
        ...
        //显示右菜单
        [self.view sendSubviewToBack:_leftViewController.view];//把_leftViewController放到父视图的最下面
        //更新右菜单的位置
...
- (void)dealWithFastSliding:(CGFloat)speedX {
    ...
    CGFloat rootX = CGRectGetMinX(_rootViewController.view.frame);
    if (swipeRight) {//向右滑动
        if (rootX > 0) {//显示左菜单
            [self showLeftViewControllerAnimated:true];
        } else if (rootX < 0) {
...
- (void)showRootViewControllerAnimated:(BOOL)animated {
    [UIView animateWithDuration:[self animationDurationAnimated:animated]
                     animations:^{
...
#pragma mark -
#pragma mark - UIPanGestureRecognizerDelegagte
